...
const CATEGORIAS_PADRAO = [
 ...
 { nome: 'Viagem', emoji: '', tipo: 'despesa' },
...
Saída: [{"tipo":"despesa","valor":50,"descricao":"comida","categoria":"Alimentação"},{"tipo":"despesa","valor":30,"descricao":"uber","categoria":"Transporte"},{"tipo":"despesa","valor":10,"descricao":"almoço","categoria":"Alimentação"}] Entrada: "paguei 120 de luz e recebi 3000 de salário"
Saída: [{"tipo":"despesa","valor":120,"descricao":"conta de luz","categoria":"Casa"},{"tipo":"receita","valor":3000,"descricao":"salário","categoria":"Salário"}]`; const SYSTEM_PROMPT_DIVIDA = `Você é o assistente financeiro do Seu Bolso.
Analise a mensagem e retorne APENAS JSON, sem markdown, sem explicação.
Hoje é: {DATA_HOJE}.
...
- "Recebi salário" → null
- "Conta de luz 120" → null Para data_vencimento: converta "dia 30", "dia 15/10", "fim do mês", "semana que vem" para YYYY-MM-DD usando OBRIGATORIAMENTE o ano de {ANO_ATUAL}. Se não houver data clara, use null.`; // ── SYSTEM PROMPT — Agenda ──────────────────────────────────────────────────
...
// Auth PostgreSQL
// ────────────────────────────────────────────────────────────────────────────
async function usePostgresAuthState() { await db.query(`CREATE TABLE IF NOT EXISTS whatsapp_session (
 ...
 await db.query( `INSERT INTO whatsapp_session (chave, valor, atualizado_em) VALUES ($1, $2, NOW()) ON CONFLICT (chave) DO UPDATE SET valor = $2, atualizado_em = NOW()`,
 [key, valor]
 );
 }
 ...
 get: async (type, ids) => {
 const data = {};
 for (const id of ids) { const val = await readData(`key-${type}-${id}`);
 if (val) data[id] = val;
 }
 return data;
 },
 ...
 for (const [id, val] of Object.entries(ids)) { if (val) await writeData(`key-${type}-${id}`, val); else await removeData(`key-${type}-${id}`);
 }
 }
 },
 },
...
class BotGranaZen {
 ...
 async _garantirTabelaLidMap() { await db.query(`CREATE TABLE IF NOT EXISTS lid_map (
 ...
 async _garantirTabelaDividas() { await db.query(`CREATE TABLE IF NOT EXISTS dividas_receber (
 ...
 id_curto TEXT,
 criado_em TIMESTAMPTZ DEFAULT NOW()
 ) `).catch(() => {}); await db.query(`CREATE INDEX IF NOT EXISTS idx_dividas_usuario ON dividas_receber(usuario_id)`).catch(() => {});
 }

 // ── Tabela de agenda ────────────────────────────────────────────────────
 async _garantirTabelaAgenda() { await db.query(`CREATE TABLE IF NOT EXISTS agenda (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
 usuario_id UUID NOT NULL,
 ...
 origem TEXT DEFAULT 'whatsapp',
 criado_em TIMESTAMPTZ DEFAULT NOW()
 ) `).catch(() => {}); await db.query(`CREATE INDEX IF NOT EXISTS idx_agenda_usuario ON agenda(usuario_id)`).catch(() => {}); await db.query(`CREATE INDEX IF NOT EXISTS idx_agenda_data ON agenda(data_hora)`).catch(() => {});
 }

 async _garantirCategoriasPadrao(usuarioId) { await db.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_categorias_usuario_nome
 ON categorias (usuario_id, LOWER(nome))
 WHERE usuario_id IS NOT NULL `).catch(() => {});
 for (const cat of CATEGORIAS_PADRAO) {
 await db.query( `INSERT INTO categorias (usuario_id, nome, tipo) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`,
 [usuarioId, cat.nome, cat.tipo]
 ).catch(() => {
 return db.query( `INSERT INTO categorias (usuario_id, nome) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
 [usuarioId, cat.nome]
 ).catch(() => {});
 });
 }
 ...
 lidCache.set(remoteJid, telefone);
 await this._garantirTabelaLidMap();
 await db.query( `INSERT INTO lid_map (lid, telefone) VALUES ($1, $2) ON CONFLICT (lid) DO UPDATE SET telefone = $2`,
 [remoteJid, telefone]
 );
 return telefone;
 }
 ...
 if (loggedOut) {
            console.warn('🚪 Sessão encerrada. Limpando credenciais...'); try { await db.query(`DELETE FROM whatsapp_session`); } catch {}
 this._tentativas = 0;
 ...
 await this.enviar(remoteJid, ' Ocorreu um erro. Tente novamente em instantes.');
 }
 }
 });
 ...
 const { rows } = await db.query(`SELECT a.*, s.telefone
 FROM agenda a
 ...
 try { const jid = `55${comp.telefone}@s.whatsapp.net`;
 ...
 ? ' *AGORA!*'
 : diffMin < 60 ? `em *${diffMin} minutos*` : `em *${Math.round(diffMin / 60)}h${diffMin % 60 > 0 ? diffMin % 60 + 'min' : ''}*`;
 ...
 await db.query( `UPDATE agenda SET lembrete_enviado = true WHERE id = $1`,
 [comp.id]
 );
 ...
 if (!transcricao) return this.enviar(remoteJid, ' Não consegui entender o áudio. Tente enviar texto.');
     console.log(`🎙️ Transcrição: ${transcricao}`); await this.enviar(remoteJid, `_Entendi: "${transcricao}"_`);
 await this.processarTexto(remoteJid, usuarioId, nome, transcricao, telefone);
 } else if (tipo === 'imagem') {
 await this.enviar(remoteJid, ' Recebi sua imagem! Analisando...');
 ...
 } else { await this.enviar(remoteJid, `Só consigo processar *texto*, *áudio* e *imagens*.\n\nDigite *ajuda* para ver como usar.`);
 }
 }

 async _buscarSessao(telefone, remoteJid = null, pushName = null) { await db.query(`ALTER TABLE sessoes_bot ADD COLUMN IF NOT EXISTS lid TEXT`).catch(() => {});

 const variacoes = this._gerarVariacoesTelefone(telefone);
 let res = { rows: [] };

 for (const tel of variacoes) {
 res = await db.query( `SELECT s.usuario_id, u.nome, s.telefone FROM sessoes_bot s JOIN usuarios u ON u.id = s.usuario_id WHERE s.telefone = $1`,
 [tel]
 );
 if (res.rows.length > 0) {
 if (tel !== telefone) { await db.query(`UPDATE sessoes_bot SET telefone = $1 WHERE telefone = $2`, [telefone, tel]).catch(() => {}); await db.query(`UPDATE usuarios SET telefone = $1 WHERE telefone = $2`, [telefone, tel]).catch(() => {});
 }
 break;
 }
 }
 ...
 res = await db.query( `SELECT s.usuario_id, u.nome, s.telefone FROM sessoes_bot s JOIN usuarios u ON u.id = s.usuario_id WHERE s.lid = $1`,
 [remoteJid]
 );
 }
 ...
 res = await db.query( `SELECT s.usuario_id, u.nome, s.telefone FROM sessoes_bot s
 JOIN usuarios u ON u.id = s.usuario_id WHERE LOWER(u.nome) LIKE LOWER($1) LIMIT 1`, [`${primeiroNome}%`]
 );
 if (res.rows.length > 0) {
 ...
 lidCache.set(remoteJid, telEncontrado); await db.query(`UPDATE sessoes_bot SET lid = $1 WHERE telefone = $2`, [remoteJid, telEncontrado]).catch(() => {});
 } catch {}
 }
 ...
 } else { await this.enviar(remoteJid, `Encontrei *${transacoes.length} transações*. Registrando...`);
 const registradas = [];
 ...
 const fmt = (v) => Number(v).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); let msg = `*${registradas.length} transações registradas!*\n\n`;
 let totalDespesas = 0, totalReceitas = 0;
 for (const tx of registradas) {
  const emojiCat = EMOJI_CATEGORIA[tx.categoria] || ''; msg += `${emoji} *${tx.descricao}* — ${fmt(tx.valor)}\n`; msg += ` ${emojiCat} ${tx.categoria} | *${tx.idCurto}*\n\n`;
 ...
 if (!conteudo || conteudo === 'null') return null; const parsed = JSON.parse(conteudo.replace(/```json|```/g, '').trim());
 ...
 let tentativas = 0;
 do {
 idCurto = gerarIdCurto(); const existe = await db.query(`SELECT id FROM agenda WHERE id_curto = $1`, [idCurto]);
 ...
 await db.query( `INSERT INTO agenda (usuario_id, titulo, data_hora, lembrar_antes, local, notas, id_curto, origem) VALUES ($1, $2, $3, $4, $5, $6, $7, 'whatsapp')`,
 ...
 const dataFmt = dataHora.toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo', day: '2-digit', month: '2-digit', year: 'numeric' });
 const horaFmt = dataHora.toLocaleTimeString('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit' });
 const lembrarLabel = compromisso.lembrar_antes >= 60 ? `${compromisso.lembrar_antes / 60}h antes` : `${compromisso.lembrar_antes} minutos antes`;
 ...
 const { rows } = await db.query( `SELECT id_curto, titulo, data_hora, local, notas, lembrar_antes, lembrete_enviado
 FROM agenda
 ...
 let tempoLabel = '';
 if (diffMin < 0) {
 statusEmoji = ''; tempoLabel = `_(já passou)_`;
 } else if (diffMin < 60) {
 statusEmoji = ''; tempoLabel = `_(em ${diffMin}min)_`;
 } else if (diffMin < 1440) {
 statusEmoji = ''; tempoLabel = `_(em ${Math.round(diffMin / 60)}h)_`;
 }
 msg += `${statusEmoji} *${comp.titulo}*${tempoLabel}\n`; msg += ` ${dataFmt} às ${horaFmt}`; if (comp.local) msg += ` | ${comp.local}`; msg += `\n *${comp.id_curto}*\n\n`;
 }
 ...
 const { rows } = await db.query( `UPDATE agenda SET cancelado = true
 WHERE usuario_id = $1 AND UPPER(id_curto) = $2 AND cancelado = false RETURNING titulo, data_hora`,
 [usuarioId, idCurto]
 );

 if (rows.length === 0) {
 return this.enviar(remoteJid, `Compromisso *${idCurto}* não encontrado ou já cancelado.\n\nDigite _"agenda"_ para ver os ativos.`
 );
 }
 ...
 await this.enviar(remoteJid, `*Compromisso cancelado!*\n\n` + ` ${comp.titulo}\n` + ` ${dataFmt} às ${horaFmt}\n\n` + `Digite _"agenda"_ para ver os demais.`
 );
 }
 ...
 do {
 idCurto = gerarIdCurto(); const existe = await db.query(`SELECT id FROM dividas_receber WHERE id_curto = $1`, [idCurto]);
 if (existe.rows.length === 0) break;
 tentativas++;
 } while (tentativas < 20);
 ...
 await db.query( `INSERT INTO dividas_receber (usuario_id, devedor, descricao, valor, data_vencimento, origem, mensagem_raw, id_curto) VALUES ($1, $2, $3, $4, $5, 'whatsapp', $6, $7)`, [usuarioId, divida.devedor, divida.descricao || `${divida.devedor} te deve`, divida.valor, vencimento, textoOriginal, idCurto]
 );
 ...
 await this.enviar(remoteJid, `*Dívida registrada!*\n\n` + `Devedor: *${divida.devedor}*\n` + `Valor: *${valorFmt}*\n` + `Vencimento: ${vencimentoFmt}\n` + `Descrição: ${divida.descricao || '—'}\n\n` + `━━━━━━━━━━━━━━━━━━━━\n` + `ID: *${idCurto}*\n\n` + `Quando receber, diga:\n_"recebido ${idCurto}"_\n\n` + `Ver todas: _"a receber"_`
 );
 }

 async enviarDividasReceber(remoteJid, usuarioId) {
 await this._garantirTabelaDividas();
 const { rows } = await db.query( `SELECT id_curto, devedor, valor, data_vencimento, criado_em FROM dividas_receber
 WHERE usuario_id = $1 AND status = 'pendente' ORDER BY data_vencimento ASC NULLS LAST, criado_em DESC`,
 [usuarioId]
 );
 const { rows: totais } = await db.query( `SELECT
 ...
 const dataFmt = venc.toLocaleDateString('pt-BR'); if (diffDias < 0) { vencLabel = `Venceu ${dataFmt} (${Math.abs(diffDias)}d atrás)`; alertEmoji = ' '; } else if (diffDias === 0) { vencLabel = `Vence HOJE`; alertEmoji = ' '; } else if (diffDias <= 3) { vencLabel = `Vence em ${diffDias}d — ${dataFmt}`; alertEmoji = ' '; } else { vencLabel = ` ${dataFmt}`; }
 ...
 const { rows } = await db.query( `UPDATE dividas_receber SET status = 'recebido', data_recebimento = CURRENT_DATE WHERE usuario_id = $1 AND UPPER(id_curto) = $2 AND status = 'pendente' RETURNING *`,
 [usuarioId, idCurto]
 );
 if (rows.length === 0) return this.enviar(remoteJid, `Dívida *${idCurto}* não encontrada ou já quitada.\n\nDigite _"a receber"_ para ver as pendentes.`);
 const d = rows[0]; const contaRes = await db.query(`SELECT id, nome FROM contas WHERE usuario_id = $1 AND padrao = true LIMIT 1`, [usuarioId]);
 const contaId = contaRes.rows[0]?.id || null;
 const contaNome = contaRes.rows[0]?.nome || 'carteira';
 await db.query( `INSERT INTO transacoes (usuario_id, tipo, descricao, valor, conta_id, data_vencimento, data_pagamento, pago, origem) VALUES ($1, 'receita', $2, $3, $4, CURRENT_DATE, CURRENT_DATE, true, 'whatsapp')`, [usuarioId, `Recebido de ${d.devedor}`, d.valor, contaId]
 ); if (contaId) await db.query(`UPDATE contas SET saldo = saldo + $1 WHERE id = $2`, [d.valor, contaId]);
 const fmt = (v) => parseFloat(v).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
 await this.enviar(remoteJid, `*Recebimento confirmado!*\n\n Devedor: *${d.devedor}*\n Valor: *${fmt(d.valor)}*\n\n` + `Receita de *${fmt(d.valor)}* registrada!\n Conta: ${contaNome}\n\n` + `Digite _"a receber"_ para ver as demais pendentes.`
 );
 }
 ...
 this._estadosCategoriaFluxo.set(telefone, { etapa: 'aguardando_nome' }); await this.enviar(remoteJid, `*Nova Categoria*\n\nQual será o nome da nova categoria?\n\n_Ex: Pets, Jogos, Presente_`);
 }
 ...
 return this.enviar(remoteJid, ' Nome inválido. Use entre 2 e 50 caracteres.');
 estado.nomeCategoria = nomeCategoria;
 estado.etapa = 'aguardando_confirmacao';
 this._estadosCategoriaFluxo.set(telefone, estado); await this.enviar(remoteJid, `Confirma a criação da categoria *"${nomeCategoria}"*?\n\nResponda *sim* para confirmar ou *não* para cancelar.`);
 } else if (estado.etapa === 'aguardando_confirmacao') {
 const resp = texto.toLowerCase().trim();
 if (['sim', 's', 'yes', 'confirmar'].includes(resp)) {
 try { await db.query(`INSERT INTO categorias (usuario_id, nome) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [sessao.usuarioId, estado.nomeCategoria]);
 this._estadosCategoriaFluxo.delete(telefone); await this.enviar(remoteJid, `Categoria *"${estado.nomeCategoria}"* criada com sucesso!\n\nAgora você pode usá-la ao registrar transações.`);
 } catch (err) {
 ...
 const { rows } = await db.query( `SELECT DISTINCT ON (LOWER(nome)) nome FROM categorias WHERE usuario_id = $1 ORDER BY LOWER(nome) ASC`,
 [usuarioId]
 );
 if (rows.length === 0) return this.enviar(remoteJid, ' Você ainda não tem categorias cadastradas.'); let msg = `*Suas Categorias:*\n\n`;
 for (const row of rows) {
 const emoji = EMOJI_CATEGORIA[row.nome] || ''; msg += `${emoji} ${row.nome}\n`;
 ...
 try {
 const { rows } = await db.query( `SELECT id, descricao, valor, tipo, conta_id, id_curto FROM transacoes WHERE usuario_id = $1 ORDER BY criado_em DESC LIMIT 1`,
 [usuarioId]
 );
 ...
 try { await db.query(`ALTER TABLE transacoes ADD COLUMN IF NOT EXISTS id_curto TEXT`).catch(() => {});
 const { rows } = await db.query( `SELECT id, descricao, valor, tipo, conta_id FROM transacoes WHERE usuario_id = $1 AND UPPER(id_curto) = $2`,
 [usuarioId, idCurto]
 );
 if (rows.length === 0) {
 return this.enviar(remoteJid, `Transação *${idCurto}* não encontrada.\n\nDicas:\n• Verifique o ID\n• Digite *histórico* para ver suas últimas\n• Para excluir a última: _excluir última_`
 );
 }
 const tx = rows[0];
 const fmt = (v) => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
 ...
 await this.enviar(remoteJid, `*Transação excluída com sucesso!*\n\n Descrição: ${tx.descricao}\n Valor: ${fmt(tx.valor)}\n ID: ${idCurto}\n\n Saldo atualizado.\n\nDigite *resumo* para ver seu saldo atual.`
 );
 } catch (err) {
 ...
 async enviarUltimasTransacoes(remoteJid, usuarioId) { await db.query(`ALTER TABLE transacoes ADD COLUMN IF NOT EXISTS id_curto TEXT`).catch(() => {});
 const { rows } = await db.query( `SELECT t.id_curto, t.descricao, t.valor, t.tipo, c.nome AS categoria, t.data_pagamento
 FROM transacoes t LEFT JOIN categorias c ON c.id = t.categoria_id WHERE t.usuario_id = $1 ORDER BY t.criado_em DESC LIMIT 5`,
 [usuarioId]
 );
 if (rows.length === 0) return this.enviar(remoteJid, ' Nenhuma transação registrada ainda.');
 const fmt = (v) => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }); let msg = `*Últimas transações:*\n\n`;
 for (const tx of rows) {
  const data = tx.data_pagamento
 ? new Date(tx.data_pagamento).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' })
 : '—'; msg += `${emoji} *${tx.descricao}* — ${fmt(tx.valor)}\n`; msg += ` ${tx.categoria || 'Outros'} | ${data}`; if (tx.id_curto) msg += ` | *${tx.id_curto}*`; msg += `\n\n`;
 ...
 const mesesNome = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro']; await this.enviar(remoteJid, `Gerando seu relatório de *${mesesNome[mes-1]}/${ano}*...\n\nAguarde alguns instantes `);
 try {
 limparPdfsAntigos();
 const { outputPath, dados } = await gerarRelatorio(usuarioId, mes, ano);
 const totalTx = dados.transacoes.length;
 if (totalTx === 0) return this.enviar(remoteJid, `Não há transações em *${mesesNome[mes-1]}/${ano}* para gerar relatório.\n\nComece registrando um gasto!`);
 ...
 let tmpFile = null;
 try {
 const buffer = await downloadMediaMessage(msg, 'buffer', {}); tmpFile = path.join(TMP_DIR, `audio_${Date.now()}.ogg`);
 ...
 const resp = await axios.post('https://api.openai.com/v1/audio/transcriptions', form, { headers: { ...form.getHeaders(), Authorization: `Bearer ${process.env.OPENAI_API_KEY}` },
 timeout: 30000,
 });
 ...
 { role: 'user', content: [ { type: 'image_url', image_url: { url: `data:${mimetype};base64,${base64}`, detail: 'low' } },
 ...
 const conteudo = resp.data.choices[0].message.content.trim();
 if (conteudo === 'null' || !conteudo) return null; const parsed = JSON.parse(conteudo.replace(/```json|```/g, '').trim());
 ...
 messages: [{ role: 'system', content: SYSTEM_PROMPT }, { role: 'user', content: texto }], }, { headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 15000 });
 const conteudo = resp.data.choices[0].message.content.trim();
 if (conteudo && conteudo !== 'null') { const parsed = JSON.parse(conteudo.replace(/```json|```/g, '').trim());
 ...
 await this._garantirCategoriasPadrao(usuarioId); await db.query(`ALTER TABLE transacoes ADD COLUMN IF NOT EXISTS id_curto TEXT`).catch(() => {});

 let categoriaId = null;
 let categoriaNome = transacao.categoria || 'Outros';
 if (categoriaNome) {
 const catRes = await db.query( `SELECT id, nome FROM categorias WHERE (usuario_id = $1 OR usuario_id IS NULL) AND LOWER(nome) = LOWER($2) LIMIT 1`,
 ...
 let idCurto; let tentativas = 0;
 do {
 idCurto = gerarIdCurto(); const existe = await db.query(`SELECT id FROM transacoes WHERE id_curto = $1`, [idCurto]);
 if (existe.rows.length === 0) break;
 tentativas++;
 } while (tentativas < 20);
 ...
 const { rows } = await db.query( `INSERT INTO transacoes (usuario_id, tipo, descricao, valor, categoria_id, conta_id, data_vencimento, data_pagamento, pago, origem, mensagem_raw, id_curto) VALUES ($1,$2,$3,$4,$5,$6,CURRENT_DATE,CURRENT_DATE,true,'whatsapp',$7,$8) RETURNING id`,
 ...
 await this.enviar(remoteJid, `*Transação registrada com sucesso!*\n\n` + `Descrição: ${transacao.descricao}\n Valor: ${valorFmt}\n Tipo: ${labelTipo}\n` + `Categoria: ${categoriaNome}\n Conta: ${contaNome}\n Data: ${dataHoje}\n\n` + `━━━━━━━━━━━━━━━━━━━━\n` + `ID: *${idCurto}*\n\n` + `Errou? Diga: _"excluir ${idCurto}"_ ou _"excluir última"_`
 );
 }
 ...
 FROM transacoes
 WHERE usuario_id = $1
 AND EXTRACT(MONTH FROM COALESCE(data_pagamento, data_vencimento)) = $2 AND EXTRACT(YEAR FROM COALESCE(data_pagamento, data_vencimento)) = $3`,
 [usuarioId, mes, ano]
 );

 const { rows: catDespesas } = await db.query( `SELECT c.nome, SUM(t.valor) AS total FROM transacoes t LEFT JOIN categorias c ON c.id = t.categoria_id
 WHERE t.usuario_id = $1 AND t.tipo = 'despesa'
 AND EXTRACT(MONTH FROM COALESCE(t.data_pagamento, t.data_vencimento)) = $2
 AND EXTRACT(YEAR FROM COALESCE(t.data_pagamento, t.data_vencimento)) = $3 GROUP BY c.nome ORDER BY total DESC LIMIT 5`,
 [usuarioId, mes, ano]
 );

 const { rows: catReceitas } = await db.query( `SELECT c.nome, SUM(t.valor) AS total FROM transacoes t LEFT JOIN categorias c ON c.id = t.categoria_id
 ...
 const { rows: dividasPend } = await db.query( `SELECT COUNT(*) AS qtd, COALESCE(SUM(valor), 0) AS total FROM dividas_receber WHERE usuario_id = $1 AND status = 'pendente'`,
 [usuarioId]
 ).catch(() => ({ rows: [{ qtd: 0, total: 0 }] }));

 // Próximos compromissos para incluir no resumo
 const { rows: proximosComps } = await db.query( `SELECT titulo, data_hora FROM agenda
 WHERE usuario_id = $1 AND cancelado = false AND data_hora >= NOW() ORDER BY data_hora ASC LIMIT 3`,
 [usuarioId]
 ).catch(() => ({ rows: [] }));
 ...
 const horaFmt = dh.toLocaleTimeString('pt-BR', { timeZone: 'America/Sao_Paulo', hour: '2-digit', minute: '2-digit' }); msg += `*${comp.titulo}* — ${dataFmt} às ${horaFmt}\n`;
 ...
 async enviar(remoteJid, texto) {
   if (!this.socket || !this.conectado) { console.warn(`Bot desconectado, não enviou para ${remoteJid}`); return; } const jid = remoteJid.includes('@') ? remoteJid : `${remoteJid}@s.whatsapp.net`;
 try {
 await Promise.race([
 ...
 await this._garantirCategoriasPadrao(usuarioId); const jid = `55${telefone}@s.whatsapp.net`;
 try {
 ...
 await this._garantirTabelaLidMap(); await db.query(`INSERT INTO lid_map (lid, telefone) VALUES ($1, $2) ON CONFLICT (lid) DO UPDATE SET telefone = $2`, [jidReal, telefone]); await db.query(`ALTER TABLE sessoes_bot ADD COLUMN IF NOT EXISTS lid TEXT`).catch(() => {}); await db.query(`UPDATE sessoes_bot SET lid = $1 WHERE usuario_id = $2`, [jidReal, usuarioId]);
...
function textoLower(t) { return (t || '').toLowerCase().trim(); } module.exports = BotGranaZen;
